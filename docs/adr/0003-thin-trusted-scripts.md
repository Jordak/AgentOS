...
More complex scripts are allowed when the complexity itself provides deterministic value that must be executable, such as a validator, installer, exporter, mirror synchronizer, or benchmark runner. That complexity should remain narrowly scoped, testable, and explicit about mutability. Trust is earned by making behavior legible and bounded, not by moving every possible judgment into code.
