...
1. Establish the target:
   ...
   - Record the initial Recovery Record: issue URL, repository, integration branch, current phase, Authorization Boundary, model and effort policy, actual model and effort if reported, selection source, override notes, `unknown` or `not reported` fallbacks, known human-review or closure blockers, and next action.
...
7. Report the Workflow Result:
   - Include issue URL and final labels, integration branch, model and effort metadata with `unknown` or `not reported` fallbacks, evidence checked, acceptance-criteria reconciliation, checklist edits made or proposed, closure action or blocker, validation signals, open risks, and recommended next action.
...
- model and effort policy, actual model/effort metadata, selection source, override notes, and `unknown` or `not reported` fallbacks;
...
11. Confirm the Workflow Result names model and effort metadata with `unknown` or `not reported` fallbacks, fulfilled criteria, unmet criteria, checklist mutations, closure state, validation, open risks, and recommended Calling Workflow action.
