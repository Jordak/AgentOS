...
4. Invoke or resume `coordinate-issue-batch`:
   ...
   - After launch, let the loop go idle until the coordinator returns its Workflow Result. Once the coordinator has acknowledged setup or is simply active, repeated `read_thread` checks for coordinator progress are live polling rather than bootstrap; stop with a durable waiting-for-callback state unless timeout, recovery, or diagnostic polling has a recorded reason and bound.
