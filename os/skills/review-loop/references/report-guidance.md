...
1. Executive summary:
   - PR link, final status, base, head branch, final head commit, merge state when known, commit count, review-pass count, reviewer count, model and effort metadata with `unknown` or `not reported` fallbacks, issue-family count, terminal condition, unresolved ask-user blockers, user-declined/accepted-risk decisions, and residual risks.
...
5. Review-loop convergence:
   - Show review-pass cycles, reviewer aliases, model and effort metadata with `unknown` or `not reported` fallbacks, reviewer continuity mode and opaque handle availability for verification passes, auto-fix issue families, user-approved-fix ask-user decisions, unresolved ask-user blockers, user-declined/accepted-risk decisions, autopilot classification counts, complexity posture summaries, fix commits, and final adjudicated-clean status. Include source aliases when useful, but never expose opaque reviewer handles.
