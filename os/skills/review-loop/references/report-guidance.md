...
1. Executive summary:
   - PR link, final status, base, head branch, final head commit, merge state when known, commit count, review-pass count, reviewer count, issue-family count, terminal condition, unresolved ask-user blockers, and residual risks.
...
5. Review-loop convergence:
   - Show review-pass cycles, reviewer aliases, reviewer continuity mode and opaque handle availability for verification passes, auto-fix issue families, unresolved ask-user blockers, autopilot classification counts, complexity posture summaries, fix commits, and final adjudicated-clean status. Include source aliases when useful, but never expose opaque reviewer handles.
