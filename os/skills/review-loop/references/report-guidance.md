...
1. Executive summary:
   - PR link, final status, base, head branch, final head commit, merge state when known, commit count, review-pass count, reviewer count, model and effort metadata when available, issue-family count, terminal condition, unresolved ask-user blockers, user-declined/accepted-risk decisions, and residual risks.
