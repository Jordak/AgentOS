...
- Fill every pass request explicitly with the common fields: target, repository, base, head or current head, mode, baseline intent, model and effort metadata with `unknown` or `not reported` fallbacks, reviewer count or risk posture, optional lens overrides, custom lens notes, and reporting mode.
