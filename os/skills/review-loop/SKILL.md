...
- If `review-pass` cannot obtain clean-context reviewers, stop before convergence or ready marking and return a recoverable blocked/needs-human result. Do not treat same-context self-review as review-loop evidence.
...
- When clean-context reviewers are unavailable, record the assembled reviewer request or prompt packet, return a blocked/needs-human result, and do not post ready-for-human markers.
...
1. Establish the target:
   ...
   - Initialize a minimal loop ledger or recovery record before readiness preflight with target, base, head, reporting mode, readiness source candidates, effort metadata when available or relevant, current phase, and next safe action.
   - For feature-sized targets, apply the implementation-readiness preflight by invoking or following `ensure-implementation-readiness` in check-only mode against the issue, PR body, ADR, or local design doc, passing effort metadata when available or relevant. Continue only when check-only returns `Ready to Implement` or `Gate Skipped`; the bypass reason may already be recorded, but the verdict still comes from the readiness check. If check-only returns `Needs Design Consensus`, update the initialized ledger or recovery record before reviewer spawning, create the temporary HTML report when reporting mode allows or record the report-skipped reason, and return `blocked` with check-only mode, source reviewed, effort metadata when available or relevant, verdict, missing consensus evidence, gate-skip state, label state, and next repair owner: `ensure-implementation-readiness`.
...
2. Set the loop ledger:
   - Track readiness-preflight mode, source reviewed, effort metadata when available or relevant, verdict, missing consensus evidence, gate-skip state, label state, next repair owner, and early blocked status when readiness fails before reviewers spawn. For preflight-blocked runs, those fields live in the initialized ledger or recovery record before the terminal `blocked` result. When review proceeds, track each pass cycle, pass mode, prescribed model/effort for the orchestrator and `review-pass` invocations when available, prescription source, effective model/effort and effective source/status when observable, meaningful prescribed/effective mismatches, reviewer continuity mode, opaque handle availability, review packet path or chat status, reviewer aliases when supplied by `review-pass`, raw reviewer findings or crosswalk summaries, normalized family IDs, autopilot classification (`auto-fix`, `auto-decline`, or `ask-user`), accepted/declined/user-decision status, `ask-user` decision state, unresolved `ask-user` blockers, lazy-human brief status, complexity posture, chosen smallest closing move, fix commits, validation results, consolidated comment URL or chat status, and pass closure status.
...
14. Confirm verification review-pass requests included autopilot classifications and rationales, complexity posture, smallest closing moves or lazy-human decisions, accepted fixes, declined rationales, effort metadata when available or relevant, and validation results when applicable.
