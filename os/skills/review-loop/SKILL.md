...
- When `implement-github-issue` invokes `review-loop` under its normal contract, that invocation is equivalent delegated review/fix-loop authorization for `review-loop` to request read-only `review-pass` reviewer subagents when the harness supports them.
