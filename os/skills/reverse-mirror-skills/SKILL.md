...
4. Recommend dispositions:
   ...
   - Use these recommendations: `delete-local`, `replace-with-existing-agentos-skill`, `promote-to-personal-overlay`, `promote-to-core`, and `leave-local`.
   - Order rows by priority: local deletion candidates, replacement/refresh candidates, Personal Overlay candidates, Core candidates, then leave-local entries.
   - Include columns: `skill`, `status`, `recommendation`, `reason`, `risk`, `follow-up`, and `next action`.
   - Use follow-up values such as `none`, `core-sanitization-issue`, `manifest-design-blocked`, and `manual-review-issue`.
