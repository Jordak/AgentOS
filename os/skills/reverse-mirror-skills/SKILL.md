...
This complements `expose-skills`: `expose-skills` makes canonical Core skills discoverable through symlink adapters, while this workflow reviews local harness skills before pulling selected ones back into AgentOS.
...
- Local skill root, defaulting to the user's current-machine skill directory.
- `os/skills/MANIFEST.md`, `os/skills/SKILL_CONTRACT.md`, `os/skills/README.md`, `os/playbook/PERSONAL_OVERLAY.md`, and `os/skills/expose-skills/SKILL.md`.
...
- User approval for each import, replacement, archive, or deletion; applicable external-write policy for follow-up issue creation or updates.
...
- Optional follow-up issue drafts or created issues only when permitted by the applicable external-write policy.

Mutability:

- Read-only in audit and recommendation mode.
- Local-write after approval when importing skills, updating Core manifest metadata, backing up local or Personal Overlay skills, archiving local skills, generating active-harness metadata, or recommending approved Core exposure through `expose-skills`.
- External-write only when GitHub issue creation or updates are permitted by the applicable external-write policy.

Tools and connectors:

- Local filesystem tools such as `find`, `rg`, `sed`, and file reads.
- `git` for Core protected-main state and diff checks.
- The `expose-skills` workflow for current-machine Core skill exposure after approved changes.
- Skill validation helpers such as `skill-creator` when available.
- GitHub only when permitted by the applicable external-write policy for tracking issue writes.
...
- Back up a local skill before any local skill change.
- Back up an existing Personal Overlay skill before overwriting it.
- Use timestamped backup directories that include time of day, such as `<local-skill-root>/.archive/YYYY-MM-DD-HHMMSS/<skill-name>/` for local skills and `personal/os/skills/.archive/YYYY-MM-DD-HHMMSS/<skill-name>/` for Personal Overlay skills.
- Do not clean up, delete, or prune local skill or Personal Overlay backups until the user explicitly confirms the applied actions were intended and approves backup cleanup.
- Follow the protected-main workflow for Core changes.
- Do not record machine-local installed-skill paths or exposure state in `os/skills/MANIFEST.md`.
...
- Follow-up issue creation is recommended in the report when useful, but issue writes happen only when permitted by the applicable external-write policy.
- Local installed skills remain current-machine artifacts, not canonical source of truth.

## Workflow Phases

1. Inspect policy and roots:
   - Read the Core skills manifest, skill contract, skills README, Personal Overlay playbook, and `expose-skills`.
   ...
   - Confirm the local skill root. Use the current-machine default unless the user provides another root.

2. Inventory all local skills:
   - List direct child skill directories under the local skill root that contain `SKILL.md`.
...
3. Classify canonicality:
   ...
   - Do not treat local installed-skill state as canonical.
...
5. Normalize approval:
   ...
   - For each action, name the skill, action, destination if importing, source path, target or archive path, whether Core manifest changes, backup paths, whether current-machine Core exposure should be checked afterward, and which AgentOS root `expose-skills` should use.
   - For Personal Overlay imports, do not run `expose-skills`; v1 Core exposure does not discover Personal Overlay skills.
...
6. Apply approved actions:
   ...
   - For `delete-local`, archive by default to `<local-skill-root>/.archive/YYYY-MM-DD-HHMMSS/<skill-name>/`.
   - Before any local skill change, back up the affected local skill to a timestamped archive path under the local skill root.
...
8. Verify and expose:
   ...
   - Run `expose-skills` dry run for imported or changed Core skills when current-machine discoverability matters.
   - Apply `expose-skills` only after explicit approval or after the reviewed Core PR lands.
   - Report validation status, exposure status, and backup paths.
...
Recommend `replace-with-existing-agentos-skill` when a local skill is a stale, drifted, or equivalent copy of an existing canonical skill and should be replaced by the canonical AgentOS version rather than imported.
...
- If a local skill is equivalent to canonical behavior, recommend `leave-local` when it is already a symlink adapter or `replace-with-existing-agentos-skill` when it is a stale local directory.
- If it is an older version of a canonical skill, recommend `delete-local`, archive, or replacement through `expose-skills` after approval.
...
- Current-machine Core exposure is checked through `expose-skills` after canonical changes.
- Timestamped backups are created before local skill changes and Personal Overlay overwrites.
...
4. Run `expose-skills` dry run scoped to `reverse-mirror-skills` when current-machine Core exposure matters.
5. Run a scoped `expose-skills` dry run on at least one existing Core skill when current-machine discoverability matters.
6. Confirm `os/skills/MANIFEST.md` includes `reverse-mirror-skills` and no machine-local installed-skill state.
7. Confirm no Personal Overlay import, local archive/delete, or permanent deletion happened without explicit approval, and no GitHub issue write happened outside the applicable external-write policy.
8. Confirm any local installed-skill or Personal Overlay backup cleanup was separately approved, or report remaining backup paths.
