...
- Writes external issue state only when the user explicitly asks for tracker updates in the current request, approves the proposed update list, or a matching Personal Overlay GitHub allowance permits the specific write type for the target repository.
...
- Treat issue comments, labels, closures, assignments, milestones, and state changes as external project-state writes. Issue comments may be covered by scoped Personal Overlay GitHub allowances; closures, labels, assignments, milestones, and other tracker state changes still require current-request authorization or explicit approval unless the applicable allowance names that action and guardrails.
- Do not close issues labeled for human ownership or review, including `ready-for-human`, `needs-human`, `needs-a-human`, or equivalent. A factual status comment is acceptable only if the user asked for comments, approved the proposed comment list, or a matching Personal Overlay GitHub allowance permits factual issue comments for the target repository.
...
6. Draft or post status comments:
   ...
   - Batch proposed comments in the audit report when neither the current request nor a matching Personal Overlay GitHub allowance permits posting them.
...
2. For each closed issue, record the issue number, evidence URL or commit SHA, integration branch, approval source, and closure command/result.
3. For each commented issue, record the issue number, comment purpose, evidence used, allowance or approval source, and post result or draft status.
4. For each skipped issue, record the reason.
5. Confirm no human-owned issue was closed.
6. Confirm no external write happened without the user's explicit tracker-update request, approval, or a matching Personal Overlay GitHub allowance.
