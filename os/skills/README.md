...
- Machine-local installed harness mirrors as source of truth. Keep canonical behavior here and use `audit-skill-mirrors` to check or sync current-machine mirrors.
...
- Skill vs installed mirror: edit canonical files first, then use `audit-skill-mirrors` to update current-machine mirrors when needed.
...
When a skill comes from a repeated user request, failed run, or external recipe, record the reason in the skill or manifest. Do not record current-machine mirror paths or state in the manifest; rerun `audit-skill-mirrors` on each machine.
