...
- Run current status-refresh targets from a clean, remote-fresh git checkout when the suite depends on Git metadata or the Git index. Guidance status-eligible runs use the runner's internal HUT sanitization and are not raw-export-compatible.
