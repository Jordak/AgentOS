...
- Output artifact: concise audit report listing closed issues, commented issues, skipped issues, model and effort metadata with `unknown` or `not reported` fallbacks, evidence, and follow-up needed.
...
- Output artifact: Workflow Result naming issue URL, labels, integration branch, model and effort metadata with `unknown` or `not reported` fallbacks, evidence checked, fulfilled acceptance criteria, unmet or ambiguous acceptance criteria, checklist mutations made or proposed, closure action or blocker, validation, open risks, and recommended Calling Workflow action.
...
- Output artifact: structured Markdown review packet following `os/skills/review-pass/references/review-packet-template.md`, with target, baseline intent, model and effort metadata with `unknown` or `not reported` fallbacks, panel table, coverage, issue families, verification results, recommended dispositions, design-escape-hatch concerns, reviewer crosswalk, residual risks, reviewer continuity mode and handle availability when applicable, optional caller-private continuity handoff, and optional temporary packet path.
...
- Output artifact: temporary static HTML review-loop report with model and effort metadata with `unknown` or `not reported` fallbacks, optional `auto-fix` and `user-approved-fix` project fix commits, optional consolidated PR comments, and optional ready-for-human PR marker after no unresolved `auto-fix` or `ask-user` blockers remain.
