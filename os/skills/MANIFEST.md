...
- Output artifact: coordinator Workflow Result plus a recoverable coordinator ledger or report naming terminal status (`completed`, `blocked`, `failed`, `cancelled`, or `needs-human`), selected or provided issues, ledger location, worker branches, worktrees, PRs, worker lifecycle states and needs-human decisions, public-safe thread names and invocation references or redacted private-surface summaries, merge-event state, landing outcomes, skipped issues, blockers, release-instruction handling, validation, mutations performed, open risks, and recommended next action; return the result through a caller-supplied Workflow Invocation Reference when available; optional dedicated GitHub batch tracking issue only when explicitly authorized.
