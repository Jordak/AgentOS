...
- Output artifact: temporary static HTML review-loop report, early `blocked` result when check-only readiness fails before reviewers spawn, optional `auto-fix` and `user-approved-fix` project fix commits, optional consolidated PR comments, and optional ready-for-human PR marker after no unresolved `auto-fix` or `ask-user` blockers remain.
