...
- Tools and connectors: local filesystem, `git`, `rg`, optional GitHub connector or `gh` reads for PR metadata, review subagents when available and authorized by an explicit review-pass or reviewer-panel request, or by a caller such as `review-loop`, `os/skills/ORCHESTRATION_LOOPS.md` as the workflow-default effort lookup when caller-supplied effort metadata is absent and effort reporting is relevant, `make-temp-file` for optional packet paths, the canonical packet template under `os/skills/review-pass/references/review-packet-template.md`, per-lens prompt references under `os/skills/review-pass/references/lenses/`, and the vendored review skills as source material for lens guidance.
