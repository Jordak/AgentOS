...
- Output artifact: coordinator Workflow Result plus a recoverable coordinator ledger or report naming selected or provided issues, ledger location, worker branches/worktrees/thread names/invocation references/PRs, worker states, merge-event state, landing outcomes, skipped issues, blockers, validation, mutations performed, open risks, and recommended next action; optional dedicated GitHub batch tracking issue only when explicitly authorized.
...
- Output artifact: GitHub loop Workflow Result plus a recoverable loop Recovery Record naming repository, loop goal, mode, Authorization Boundary, loop caps, pass count, called batch-pass invocation references or public-safe summaries, called batch-pass results, merge-report state, blockers, failures, stop reason, validation, mutations performed, open risks, and recommended next action; optional dedicated GitHub tracking issue only when explicitly authorized.
