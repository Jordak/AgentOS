...
- Tools and connectors: local filesystem, `git`, `rg`, optional GitHub connector or `gh` reads for PR metadata, review subagents when available and authorized by the user or a caller such as `review-loop`, `make-temp-file` for optional packet paths, per-lens prompt references under `os/skills/review-pass/references/lenses/`, and the vendored review skills as source material for lens guidance.
