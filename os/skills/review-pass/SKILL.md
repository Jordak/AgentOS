...
- Eligible clean-context reviewers are separate clean-context or equivalent isolated reviewers that did not implement or adjudicate the target; the current implementation, parent, or same-context agent does not count as review evidence even for a read-only self-check.
...
- Count only eligible clean-context or equivalent isolated reviewers as review-pass evidence; do not count the current implementation, parent, or same-context agent even when it performs a read-only self-check.
...
4. Assemble and run prompts:
   ...
   - Spawn eligible clean-context reviewers in parallel when the harness supports it and the pass is authorized by an explicit review-pass or reviewer-panel request, or by an authorized Calling Workflow. Eligible reviewers are separate clean-context or equivalent isolated reviewers that did not implement or adjudicate the target. If eligible clean-context reviewers are unavailable, return a blocked result before review instead of running a same-context single-agent fallback. Include the assembled reviewer request or prompt packet in the blocked result.
...
- Unavailable eligible clean-context reviewer capability returns a blocked result with the reviewer request or prompt packet and no same-context review findings.
...
- Verification mode includes caller adjudication context when supplied and asks reviewers to verify it without adding new packet schema.
