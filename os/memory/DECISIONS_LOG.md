...
- AgentOS-managed files and directories should not use symlinks; Core/public content and public exports reject symlinks, and scripts must fail closed instead of following symlinks during managed reads or writes, including when the relevant managed path is in Personal Overlay. See `docs/adr/0004-no-symlinks-in-agentos-managed-files.md`.
