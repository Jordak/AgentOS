...
- Give workers a callback thread id, invocation reference, ledger surface, or equivalent result target when the harness supports one. After callback setup is acknowledged or workers are simply active, do not poll for progress; stop in a waiting-for-callback state unless timeout, recovery, or diagnostic polling has a recorded reason and bound.
