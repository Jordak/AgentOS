...
- When installed skill adapters are needed, keep the source copy in `os/skills/` and run `expose-skills` on the current machine instead of storing adapter state in portable AgentOS metadata. Use its explicit replacement mode when migrating old copied Core skill mirrors to symlink adapters.
