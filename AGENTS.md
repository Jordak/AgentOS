...
- Before implementing feature-sized AgentOS Core or publishable support-file work, run `os/skills/check-implementation-readiness/SKILL.md` against `os/playbook/IMPLEMENT_FEATURES.md`. If no durable design source exists, create or update one before coding; do not let chat-only consensus become the first implementation commit.
