...
- Make AgentOS Core or publishable support-file changes on a feature branch in an isolated worktree, open a pull request, wait for required validators, and squash merge through GitHub.
- Before implementing feature-sized AgentOS Core or publishable support-file work, run `os/skills/ensure-implementation-readiness/SKILL.md` against `os/playbook/IMPLEMENT_FEATURES.md`. If no durable design source exists, create or update one before coding, and ensure the durable source has the full readiness fields with valid consensus provenance or an explicit `Gate Skipped` record; do not let chat-only consensus become the first implementation commit.
